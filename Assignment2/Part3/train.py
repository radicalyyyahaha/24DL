...
def main(config):
    ...
    model = VanillaRNN(config.input_length, config.input_dim, config.num_hidden, config.num_classes)  # fixme
    model.to(device)
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)  # fixme
