...
class MLP(object):
    def __init__(self, n_inputs, n_hidden, n_classes):
        ...
        # Layer structure: Linear -> ReLU for each hidden layer, followed by Linear -> SoftMax for output.
        self.layers = []
        layer_sizes = [n_inputs] + n_hidden + [n_classes]
        
        # Creating Linear + ReLU layers for hidden layers
        for i in range(len(n_hidden)):
            self.layers.append(Linear(layer_sizes[i], layer_sizes[i + 1]))
            self.layers.append(ReLU())
        
        # Creating the output layer: Linear -> SoftMax
