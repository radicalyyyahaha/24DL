    ...
    with open('training_log.csv', mode='w') as log_file:
        ...
        for step in range(max_steps):
            ...
            for start_idx in range(0, num_samples, BATCH_SIZE):
                ...
                loss = cross_entropy.forward(logits, batch_y)

                dout = cross_entropy.backward(logits, batch_y)
                mlp.backward(dout)
            ...
            if step % eval_freq == 0 or step == max_steps - 1:
                ...
                train_accuracy = accuracy(logits, batch_y)
                test_accuracy = accuracy(test_logits, y_test)

                # Log training and testing loss and accuracy
                log_writer.writerow([step, loss, train_accuracy, test_loss, test_accuracy])
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=BATCH_SIZE, 
                        help='Batch size for training. If set to 1, stochastic gradient descent is used.')
